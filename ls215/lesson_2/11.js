...
function getDateObject(dates) {
  return dates.map(date => {
    let dateComponents = date.split(' ')[1].split('-');
    let year = parseInt(dateComponents[2], 10);
    let monthIndex = parseInt(dateComponents[0], 10) - 1;
...
function getDateRange(dates) {
  let dateObj = getDateObject(dates);
  dateObj.sort((a, b) => a.valueOf() - b.valueOf());
  return dateObj[0].toDateString() + ' - ' + dateObj[dateObj.length - 1].toDateString();
}

function mailCount(emailData) {
  let emails = emailData.split('##||##\n\n');
  let emailDates = emails.map(email => email.split('#/#\n')[2]);
